...
const KeywordModal = ({
  ...
  const [keywordInput, setKeywordInput] = useState(keyword.related_keyword);
  ...
  const handleDelete = () => {
    axios
      .delete(`/api/v1/manager/related-keyword/${keyword.keyword_id}`, {
        headers: {
  ...
  const handleUpdate = () => {
    if (keywordInput.length < 3) {
      setDisabled(true);
      toast.error("Keyword name should be bigger than 5 letters.", {
        autoClose: 3000,
      });
      setTimeout(() => {
        setDisabled(false);
    ...
    axios
      .put(
        `/api/v1/manager/related-keyword/${keyword.keyword_id}`,
        {
          related_keyword: keywordInput,
        },
        {
          headers: {
            Authorization: `Bearer ${auth.accessToken}`,
...
<div className="fixed top-0 left-0 w-full h-full flex justify-center items-center z-[800] bg-black bg-opacity-50 px-2">
    <div ref={modalRef} className="bg-white rounded-lg p-8 ">
      ...
      <div className="mt-5 flex flex-col md:flex-row gap-2 md:gap-16 justify-center md:items-center ">
        <label className="font-semibold text-sm">Keyword: </label>
        <input
          type="text"
          disabled={disabled}
          value={keywordInput}
