...
const SubjectModal = ({ subject, closeModal, setIsChanged, isChanged }) => {
  const modalRef = useRef(null);
  const { auth } = useAuth();
  const [disabled, setDisabled] = useState(true);
  const [subjectInput, setSubjectInput] = useState(subject.subject_name);
  ...
  const handleDelete = () => {
    axios
      .delete(`/api/v1/manager/subject/${subject.subject_id}`, {
        headers: {
  ...
  const handleUpdate = () => {
    if (subjectInput.length < 3) {
      setDisabled(true);
      toast.error("Subject name should be bigger than 5 letters.", {
        autoClose: 3000,
      });
      setTimeout(() => {
        setDisabled(false);
    ...
    axios
      .put(
        `/api/v1/manager/subject/${subject.subject_id}`,
        {
