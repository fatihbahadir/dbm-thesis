...
const AddUniversityModal = ({ closeModal, isChanged, setIsChanged }) => {
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 flex flex-col md:flex-row gap-2 md:gap-16 justify-center md:items-center ">
              ...
              <input type='text' disabled={disabled} value={university} onChange={(e)=>{setUniversity(e.target.value)}} className='border bg-[#fdfdff] border-[#e4e6fc] transition-all rounded w-[300px] outline-none focus:border-main px-3 py-2' />
          </div>

          <div className='w-full mt-5 text-right'>
              <button disabled={disabled} onClick={saveUniversity} className='py-1 disabled:bg-gray-400 px-6 bg-main hover:bg-mainHover transition text-white rounded'>
                  Save
              </button>
          </div>
    </div>
