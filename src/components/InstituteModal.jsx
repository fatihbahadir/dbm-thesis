...
const InstituteModal = ({
  ...
  const [instituteInput, setInstituteInput] = useState(institute.institute_name);
  ...
  const handleDelete = () => {
    axios
      .delete(`/api/v1/manager/institute/${institute.institute_id}`, {
        headers: {
  ...
  const handleUpdate = () => {
    ...
    axios
      .put(
        `/api/v1/manager/institute/${institute.institute_id}`,
        {
          institute_name: instituteInput,
        },
        {
          headers: {
            Authorization: `Bearer ${auth.accessToken}`,
    ...
    <div ref={modalRef} className="bg-white rounded-lg p-8 ">
      ...
      <div className="mt-5 flex flex-col md:flex-row gap-2 md:gap-16 justify-center md:items-center ">
        ...
        <input
          ...
          onChange={(e) => {
            setInstituteInput(e.target.value);
          }}
