...
const ManageTypes = () => {
  ...
  return (
    <div className='w-full h-full'>
                  {isModalOpen && (
        <AddTypeModal isChanged={isChanged} setIsChanged={setIsChanged} closeModal={closeModal}  />
      )}
      {
        selectedType &&
        <TypeModal type={selectedType} isChanged={isChanged} setIsChanged={setIsChanged} closeModal={closeTypeModal}  />

      }

        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
          {thesisParams?.types?.map((type)=>(
            <div onClick={()=>openTypeModal(type)} key={type.thesis_type_id} className='bg-white transition hover:scale-105 cursor-pointer hover:shadow-2xl shadow-lg p-5 rounded m-6 flex justify-between items-center'>
              <h2 className='tracking-wider font-bold'>{type.thesis_type_name}</h2>
